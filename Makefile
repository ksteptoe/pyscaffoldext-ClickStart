...
PYTEST_COV_BASE := --cov=pyscaffoldext
...
test-all:
	$(PYTHON) -m pytest -v -m "not live" $(PYTEST_WARN) $(PYTEST_XDIST) $(PYTEST_TIMEOUT) --cov=pyscaffoldext --cov-report=term-missing --cov-report=xml --cov-fail-under=40


# Live tests are explicit & uncached (gentle on API; clearer intent)
test-live:
	SF_LIVE_TESTS=true $(PYTHON) -m pytest -v -m live $(PYTEST_WARN) --timeout=180 --cov=pyscaffoldext --cov-report=xml
...
changelog:
	...
	@git log $(LAST_TAG)..HEAD --pretty=format:"- %s (%h)" --no-merges || echo "(no commits since $(LAST_TAG))"

changelog-md:
	@mkdir -p docs
	@echo "Writing docs/CHANGELOG.md ..."
	@printf "# Changelog\n\n## Since %s\n\n" "$(LAST_TAG)" > docs/CHANGELOG.md
	@git log $(LAST_TAG)..HEAD --pretty=format:"- %s (%h)" --no-merges >> docs/CHANGELOG.md || true
...
release-show: fetch-tags
	...
	@echo "installed dist version:"; $(PYTHON) -c "import importlib.metadata as m; print(m.version('$(PKG)'))" || echo "(package not installed)"
